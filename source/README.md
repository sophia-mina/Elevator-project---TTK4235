# Elevatorproject 
This project simulates an elevator system and was developed as a partner project in the course [TTK4235 - Embedded Systems at NTNU](https://github.com/ITK-TTK4235). The simulator used in this project can be found here: [Elevator Simulator](https://github.com/ITK-TTK4235/elevator_simulator). My partner and I implemented the logic in `main.c` and in the directories `c_files` and `header`, enabling the elevator to handle and execute orders from both internal and external buttons.
...
This project simulates the behavior of a real elevator system, based on the specifications from the TTK4235 lab assignment.
...
- Indicator lights for orders and current floor
...
- All orders are ignored until initialization is complete
...
- Orders in the current travel direction are prioritized
...
- The door opens when the elevator stops at a requested floor
...
- The door only opens when aligned with a floor
...
- The system should not crash or require restart during normal operation
- After initialization, the elevator always knows its position
...
- A queue system in form of a linked list for managing orders
